...
cmd_start() {
  ...
  local asset="btc"
  local entry_timeout_min="5"
  local stake_usd=""
  local threshold=""
  local max_entry_price=""
  local poll_sec="2"
  ...
  local -a runner_cmd
  runner_cmd=("$VENV_PY" "$RUNNER" "--profile" "$profile" "--asset" "$asset" "--entry-timeout-min" "$entry_timeout_min" "--poll-sec" "$poll_sec" "--close-retry-max" "$close_retry_max" "--close-retry-delay-sec" "$close_retry_delay_sec" "--execute")
  [[ -n "$stake_usd" ]] && runner_cmd+=("--stake-usd" "$stake_usd")
  [[ -n "$threshold" ]] && runner_cmd+=("--threshold" "$threshold")
  [[ -n "$max_entry_price" ]] && runner_cmd+=("--max-entry-price" "$max_entry_price")
